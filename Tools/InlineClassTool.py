...
Here is one implementation of a Python module that reads 
...
Example usage:
	...
	out_h = inline_class(h_file, cpp_file)
...
def inline_class(header_file, impl_file):
    ...
    with open(impl_file, 'r') as f:
        impl_text = f.read()
    # extract class name
    class_name_match = re.search(r'class\s+(\w+)\s*\n*{', header_text)
    ...
    method_impl_regex = r'^\s*void\s+' + class_name + '::(\w+)\((.*)\)\s*\n*\{\s*(.*?)\s*\}'
